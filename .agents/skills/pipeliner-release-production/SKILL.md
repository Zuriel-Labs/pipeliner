...
1. Revalidate the approval phrase, candidate identity, pull-request state, exact tree, required checks, environment, and authorization immediately before mutation.
...
4. Use only the configured build, promotion, deployment, and verification commands and the repository's declared source of truth. Never make an imperative live edit the durable fix.
