...
1. Read upstream and target instructions completely. Preserve dirty work and stronger constraints. Manually merge only adoption-authorized changes; never overwrite unknown changes.
   Review question guidance in root instructions, provider imports/adapters and prompts. Known native-tool and continued-work directives appear in the dry-run's `questionConflicts` and block writes even with matching review hashes. Reconcile authorized text to the final-message/end-turn contract before recording hashes. The detector is bounded; manually inspect other effective instructions and disclose higher-priority host conflicts without editing global settings.
