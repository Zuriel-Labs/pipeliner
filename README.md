...
All questions use ordinary agent messages marked `Question:`. The agent sends the question as its final response and ends the turn immediately, stopping all task work and tool calls until the Human replies at their own pace. Native question controls, background continuation, polling, sleep loops, timers, reminders and silence-based defaults are prohibited. Agents save previous answers before asking and reuse them on resume. Exact approval phrases and real access/host/repository controls still apply. Adoption dry-runs report known conflicting instructions as `questionConflicts`, including hash-reconciled target text, and refuse installation until authorized reconciliation is complete. Manual review still covers arbitrary prose and host instructions outside these checks.
