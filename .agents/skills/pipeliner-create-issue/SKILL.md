...
2. Resolve material ambiguity from repository evidence. When evidence is insufficient, ask focused questions with tradeoffs and a recommended default through permitted native app controls, otherwise a clearly marked message. Wait indefinitely for required answers without timers or inferred defaults; preserve existing answers. Do not invent product behavior.
